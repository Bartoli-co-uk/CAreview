...
three risks (`RISK-009` npm build-time supply chain — **new and undecided**;
`RISK-010` onboarding regression; `RISK-011` recording `DECISION-027`'s
...
| Last human decision | `DECISION-027` (ISSUE-0013 risk acceptance and merge); `DECISION-026` (ISSUE-0013 start authorization); `DECISION-025` (ISSUE-0012 advance and merge, tracked residual); `DECISION-024` (React/Vite frontend, direct-override, out-of-band); `DECISION-023` (M2 milestone acceptance); `DECISION-022` (ISSUE-0011 advance and merge); `DECISION-021` (ISSUE-0011 start authorization); `DECISION-020` (ISSUE-0010 advance and merge); `DECISION-019` (ISSUE-0009 advance and merge); `DECISION-018` (ISSUE-0009 start authorization); `DECISION-017` (ISSUE-0008 advance and merge); `DECISION-016` (ISSUE-0007 advance and merge); `DECISION-015` (roadmap v4 approval); `DECISION-014` (secret retention, RISK-002-as-widened, tenant validation); `DECISION-013` (brief v2 approval); also `DECISION-012`..`001` |
| Open blockers | **Roadmap v5 is blocked at its review gate.** `AGENTS.md` requires a fresh Codex plan review of a new roadmap version; `codex` was not available in the environment where v5 was drafted, so the launcher could not be invoked. An unavailable Codex tool blocks — it is never skipped or relabelled. v5 stays `DRAFT` until that review runs against its exact commit, or the human records an explicit decision to approve directly from the record (the route `DECISION-015` took for v4). M1/M2 remain accepted and are unaffected |
| Tracked follow-up | Live-tenant sign-in verification (M1 and M2, both auth modes) — deferred pending the human's access restrictions (`DECISION-012`), remains a protected action, not a completion gate for either milestone. `DECISION-023`'s SEC-001 (silent-renewal-after-revocation replay) and SEC-003 (unauthenticated credential-validation oracle, undocumented in `RISK-002`) are tracked, non-blocking follow-ups — see `project/milestones/M2.md` and `DECISION-023`. **CI still does not run the frontend build or its 91 Vitest tests — now tracked as `ISSUE-0014` (`PLANNED`) rather than as an ownerless note.** **`RISK-009` (npm build-time supply chain) and `RISK-010` (fresh clone no longer serves a UI without a build) are new in v5 and undecided — `RISK-009` in particular needs a human decision, and nothing claims it is mitigated.** Whether to run M3's milestone gate at all is an open question for the human (`project/milestones/M3.md`) |
| Next required actor | **The human.** Roadmap v5 needs either a Codex plan review or an explicit approve-from-record decision. Separately open for decision: `RISK-009`'s treatment, whether to start `ISSUE-0014`, and whether M3's milestone gate should run |
...
| Actions not yet permitted | Starting `ISSUE-0014` or any other issue/milestone work without an explicit human decision; treating roadmap v5 as approved, or its `RISK-009`/`RISK-010` as accepted; running M3's milestone gate without a decision to do so; reopening `ISSUE-0013`'s accepted residual without a fresh authorization; publication, deployment, live tenant auth/fetch (either mode), or any other protected action |
