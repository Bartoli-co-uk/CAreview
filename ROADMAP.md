...
**Current status:** `DRAFT` (roadmap **v6**; two Codex plan-review rounds complete, all findings fixed, planning-repair budget now used — pending the human's approval decision, see below). **v5 remains separately approved** (`DECISION-029`) and continues to govern the completed M1/M2/M3 content and the completed project-level review below; nothing in v6 changes any of it.
**Roadmap version:** `6`
...
**(v6)** This version is a draft: it adds `M4`, four issues, and one risk. It is
not yet approved and does not by itself authorize any implementation. Once
approved, approval of the roadmap still does not by itself start an issue —
that needs its own separate human decision, exactly as every prior issue's
...
### M4 issue sequence **(v6, DRAFT — not yet approved)**
...
| `RISK-012` | **(v6, DRAFT — new, needs human treatment decision before v6 approval)** `ISSUE-0018`'s `phishing-resistant-mfa-admins` rule only recognizes Microsoft's *built-in* phishing-resistant authentication-strength policy by a hardcoded ID. A tenant that instead defines a **custom** authentication strength restricted to the same phishing-resistant methods (FIDO2/certificate/Windows Hello) is arguably compliant but will score a FAIL on this rule — a false negative in the analyzer's output, not a gap in the tenant's actual posture | Low — analyzer-accuracy/reputational, not a security exposure in the tool itself | Jay (@Jay-cli) | Proposed treatment: document the limitation explicitly in `rules.py`'s rationale string and the `README.md` table row (already required by `ISSUE-0018`'s acceptance criteria); do not attempt custom-strength recognition in this pass (would need a new Graph call to resolve the custom strength's allowed methods — its own design decision). Needs the human's explicit acceptance as a residual, same as `RISK-009`/`RISK-010`/`RISK-011`, before v6 can be approved | At `ISSUE-0018`; any M4 milestone security review |
