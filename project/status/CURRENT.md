...
| Open blockers | None for `M1`–`M3` (all `COMPLETE` and accepted: `DECISION-012`, `DECISION-023`, `DECISION-032`; project-level review `COMPLETE` and accepted, `DECISION-033`); `ISSUE-0015`/`ISSUE-0016` are both `COMPLETE` and merged with their sandbox-only residuals accepted (`DECISION-037`, `DECISION-040`). `ISSUE-0017`'s round-0 candidate is implemented and locally verified (205 tests, compile, governance all passing) but has no Codex review yet. `ISSUE-0018` remains unauthorized to start |
