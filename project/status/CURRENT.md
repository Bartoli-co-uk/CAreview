...
| Field | Current value |
|---|---|
| Stage | `ISSUE_REVIEW` — `ISSUE-0012` (retroactive React/Vite frontend review, out-of-band per `DECISION-024`); M1 and M2 remain complete and accepted with no milestone in progress |
...
| Active issue | `ISSUE-0012` — React/Vite frontend, `REVIEWING`, out-of-band (not under the M1/M2 roadmap; see `DECISION-024`). No further issues are planned under the current roadmap |
| Issue repair round | Round 0 (initial review, not yet run at this point in the record) |
...
| Tracked follow-up | Live-tenant sign-in verification (M1 and M2, both auth modes) — deferred pending the human's access restrictions (`DECISION-012`), remains a protected action, not a completion gate for either milestone. `DECISION-023`'s SEC-001 (silent-renewal-after-revocation replay) and SEC-003 (unauthenticated credential-validation oracle, undocumented in `RISK-002`) are tracked, non-blocking follow-ups — see `project/milestones/M2.md` and `DECISION-023`. `DECISION-024`'s frontend work: no Codex review yet performed, CI not yet updated for `npm` commands, and whether to open an M3 for it is undecided |
| Next required actor | The human — no roadmap work is currently authorized; a new issue, milestone, or roadmap cycle needs its own explicit start. For the `DECISION-024` frontend work specifically: decide whether to request a (now-retroactive) Codex review, merge `ai/react-dashboard-frontend` to `main`, and whether to open an M3 |
| Next permitted action | **Wait**, except for the human's own next step on the `DECISION-024` frontend branch. A new task should read this file, `AGENTS.md`, and `ROADMAP.md` fresh before proposing any next step (e.g., a new roadmap cycle, merging the frontend branch, or picking up a previously deferred item such as live-tenant verification or the `DECISION-023` follow-ups) |
...
| Frontend tests (new, `DECISION-024`) | `cd frontend && npm test` | 86 passed, exit 0 |
