...
| Next required actor | The human, to authorize starting `ISSUE-0018` (roadmap v6/`M4` is approved, but no issue may start without its own separate start decision) |
| Next permitted action | **Wait for an issue-start decision**, or start `ISSUE-0018` (`M4`'s final planned issue) once the human authorizes it. A new task should read this file, `AGENTS.md`, and `ROADMAP.md` fresh before proposing any next step |
| Actions not yet permitted | Starting `ISSUE-0018` without its own separate human start authorization; starting any other new issue/milestone/roadmap work without an explicit human decision; reopening `ISSUE-0013`'s accepted residual, `ISSUE-0014`'s accepted sandbox residual, `ISSUE-0015`'s accepted sandbox residual (`DECISION-037`), `ISSUE-0016`'s accepted sandbox residual (`DECISION-040`), `ISSUE-0017`'s accepted sandbox residual (`DECISION-042`), `M3`'s accepted governance-record staleness (`DECISION-032`), the project-level review's accepted residual (`DECISION-033`), or `DECISION-028`'s `RISK-009`/`DECISION-029`'s `RISK-010`/`DECISION-034`'s `RISK-012` acceptances, without a fresh authorization; publication, deployment, live tenant auth/fetch (either mode), or any other protected action |
