...
| Field | Current value |
|---|---|
| Stage | `ISSUE_REVIEW` — roadmap v5 is `APPROVED` (`DECISION-029`, binding `8ea41ee`; round 1 `CHANGES_REQUIRED` fully repaired, round 2 `CHANGES_REQUIRED` accepted as residuals). `ISSUE-0014` (wire the frontend build/tests into CI) was start-authorized (`DECISION-030`) and its round-0 candidate is committed on `ai/ISSUE-0014-frontend-ci`, awaiting its fresh Codex issue review |
