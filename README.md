...
The rule set lives in [`rules.py`](rules.py) — twelve data-driven rules, each with a
severity, a weight, a rationale and a remediation string. Total weight is 135.
...
| [`rules.py`](rules.py) | The declarative rule set: twelve rules with severity, weight, rationale, remediation and required fields, plus the evaluability model. |
