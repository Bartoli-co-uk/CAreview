...
`ISSUE-0017`/`ISSUE-0018` are not yet authorized to start.
...
| 17 | `ISSUE-0017` | New rule `admin-signin-frequency`: full admin-role-union coverage requiring short re-authentication (everyTime, or 1–4 hours) with no persistent browser session | `ISSUE-0016` (sequencing only) | `normalize_policy` gains `signInFrequency`/`persistentBrowser` (additive, safely defaulted); rule reuses the `mfa-admins` full-coverage pattern; three test cases including the persistent-browser-always-on nuance; one new, narrowly-scoped `strong_tenant.json` fixture entry that doesn't perturb any other rule; `README.md` updated | Medium — new fixture entry, human-confirmed threshold must not be silently changed | `PLANNED` |
