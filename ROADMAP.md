...
- **(v4, `DECISION-014`) Secret lifecycle (the named security boundary of M2):**
  ...
  **retained for the app-only session's lifetime** (not discarded after the
  ...
  rendered back to the page; it is cleared on logout and on process exit, same
  as a token. Provider error text is truncated and scrubbed of any occurrence
  of the submitted secret before it is returned or logged.
