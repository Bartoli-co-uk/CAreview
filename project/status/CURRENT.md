...
| Field | Current value |
|---|---|
| Stage | `ISSUE_REVIEW`, active issue `ISSUE-0015`. Roadmap v6 is `APPROVED` (`DECISION-034`), adding `M4`: four new analyzer rules (`ISSUE-0015`–`ISSUE-0018`), researched against Microsoft's 2026 Conditional Access baseline and the CIS Microsoft 365 Foundations Benchmark, each using fields Graph already returns on the existing `identity/conditionalAccess/policies` call — three of the four (`ISSUE-0016`–`ISSUE-0018`) need `normalize_policy` extended to stop dropping fields it currently discards; `ISSUE-0015` needs no `normalize_policy` change. Both permitted Codex plan-review rounds ran (`ROADMAP-bb4952d6e6cc-*`, `ROADMAP-68655cc7b1e0-*`), all 8 findings fixed, and `RISK-012` (a documented false-negative limitation in `ISSUE-0018`'s rule) is accepted as residual in the same decision. M1/M2/M3 and the project-level review (`DECISION-033`) remain complete and accepted, unaffected. `ISSUE-0015` is now started (`DECISION-035`) and implemented on `ai/ISSUE-0015-location-restriction-rule`; its round-0 Codex review was `BLOCKED` (F-001/F-002/F-003), and this record's round-1 repair addresses all three (F-002 via `DECISION-036`'s fixture-path amendment, F-003 via `DECISION-035`, F-001 by a direct record fix), with a fresh review to follow. `ISSUE-0016`–`ISSUE-0018` remain `PLANNED`, not authorized to start |
...
| Active milestone | `M1` — `COMPLETE`, accepted (`DECISION-012`). `M2` — `COMPLETE`, accepted (`DECISION-023`). `M3` (React/TypeScript dashboard UI) — `COMPLETE`, accepted (`DECISION-032`). Project-level definition-of-done review — `COMPLETE`, accepted (`DECISION-033`). **`M4` (analyzer rule-set expansion) — `PLANNED`, roadmap v6 `APPROVED` (`DECISION-034`); its first issue, `ISSUE-0015`, is now started and in review (`DECISION-035`).** `M4` itself has not opened a milestone gate |
| Active issue | `ISSUE-0015` (analyzer rule `location-restriction-present`) — `REVIEWING`, branch `ai/ISSUE-0015-location-restriction-rule`, started under `DECISION-035`. Round 0 Codex review `BLOCKED`; round-1 repair in progress in this same task, fresh re-review pending. `ISSUE-0014` (wire the frontend build/tests into CI) is `COMPLETE` (merged, accepted sandbox execution-evidence residual, `DECISION-031`), branch `ai/ISSUE-0014-frontend-ci` merged to `main`. `ISSUE-0013` — scoped device-code abandonment — is `COMPLETE` (merged, accepted residual, `DECISION-027`), out-of-band per `DECISION-026`. `ISSUE-0012` is also `COMPLETE` (merged) |
| Issue repair round | `ISSUE-0015` round 1 open (of at most 2 permitted): repairing round-0's F-001 (candidate-identity mismatch, direct record fix), F-002 (fixture path outside allowed paths, resolved by `DECISION-036`), and F-003 (missing start-authorization record, resolved by `DECISION-035`). `ISSUE-0014`'s round-2 residual (zero content findings; blocked only by sandbox execution-evidence limits) is accepted per `DECISION-031`. `ISSUE-0013`'s round-2 residual (F-001: cleanup fails open after ~16-minute retry exhaustion) is accepted per `DECISION-027`, not eliminated. `ISSUE-0012`'s round-2 residual is accepted per `DECISION-025` |
...
| Next required actor | The human, for `ISSUE-0015`'s advance/merge decision on the round-1 outcome (`BLOCKED`, zero content findings, sandbox-only residual) |
| Next permitted action | **Wait for the human's ISSUE-0015 advance/merge decision.** Options: accept the round-1 residual as the same class already accepted for `ISSUE-0014` (`DECISION-031`) and merge; request a third look at something specific; or stop. No further repair round may start without the human choosing that path first, since content-wise the candidate has zero findings. `ISSUE-0016`–`ISSUE-0018` remain unauthorized to start |
| Actions not yet permitted | Starting `ISSUE-0016`–`ISSUE-0018` without their own separate human start authorization; starting any other new issue/milestone/roadmap work without an explicit human decision; a third `ISSUE-0015` repair round after this one and one more (round 1 is the first of at most two permitted repair rounds); reopening `ISSUE-0013`'s accepted residual, `ISSUE-0014`'s accepted sandbox residual, `M3`'s accepted governance-record staleness (`DECISION-032`), the project-level review's accepted residual (`DECISION-033`), or `DECISION-028`'s `RISK-009`/`DECISION-029`'s `RISK-010`/`DECISION-034`'s `RISK-012` acceptances, without a fresh authorization; publication, deployment, live tenant auth/fetch (either mode), or any other protected action |
