...
**Amending brief:** `project/brief/PROJECT_BRIEF.md` v2 at `98feea68b840bc2c92eda1cd46af8217555daeb5`, approved by `DECISION-013` — governs M2 below
...
- Serve on `127.0.0.1:8765` only; no local authentication beyond loopback binding (DECISION-001, as re-examined by RISK-002 below for app-only mode).
...
- Tokens held in process memory only; never persisted to disk, logs, or the repo.
  **(v4)** The same rule binds the app-only **client secret**, with a shorter
  retention window (see Architecture).
...
- **(v4 revision — narrowed)** App registration / non-device-code authentication
  ...
  Still excluded, without exception:
  - **Certificate-based client assertions.** RSA/JWT signing would require a
    third-party dependency and break the stdlib-only constraint. Secret only.
...
- **(v4) Tenant validation for app-only mode:** client-credentials has no user
  context, so `organizations`, `common`, and `consumers` are invalid authorities.
  The **server** rejects them, and requires a GUID or a DNS-style domain label,
...
- **(v4) Blast-radius statement (brief v2 Data and security):** an application
  ...
  it does not create a new boundary. The human must re-accept RISK-002 as widened
  (or require a loopback gate first) before ISSUE-0008 begins.
...
| `ISSUE-0009` | `server.py`, `tests/test_server.py` | `web/`, `graph.py`, changes to `auth.py` beyond calling it, any new dependency | README API section entry for the new endpoint |
...
- **(v4)** `M2`: README dual-mode setup and run instructions, including the exact
  prerequisite for app-only mode and the explicit statements that CAreview never
  creates an app registration, does not support certificates, and does not store
  the secret; a rotation/revocation pointer; `docs/security-boundaries.md`
  threat-model delta for the widened trust boundary; per-issue handoffs.
...
`RISK-002` as widened, `RISK-005`, `RISK-006`, and `RISK-007` each require a
human decision recorded before the issue that depends on them; none may be
accepted, broadened, or inferred by an agent.
...
This v4 delta is a **planning draft only**. It has had no independent review.
Specifically, and stated so no later reader infers otherwise:
...
- The expected sequence is: commit v4 → fresh Codex plan review at that SHA →
  ...
  task.
- The RISK-002 re-acceptance, and the brief v2 Q5/Q6 answers, are human
  decisions that gate ISSUE-0008 regardless of the plan-review outcome. A
  `PASS` from Codex would not supply them.
...
sequence, acceptance criterion, or risk. Any edit beyond a status cell is a
...
four risks, a materially widened `RISK-002`, and a narrowed non-goal. It
therefore requires its own Codex plan review and its own human decision record;
`DECISION-003` does not cover it. The approved-and-executed M1 content above is
unchanged by v4 except where explicitly marked `(v4)`.
