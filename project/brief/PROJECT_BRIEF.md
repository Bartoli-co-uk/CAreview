...
existing device-code flow, which remains the default and is unchanged.
Certificate-based client assertions are explicitly out of scope for this
amendment (see Non-goals) and are recorded as a deferred future enhancement,
not implemented here. Everything else in v1 is unchanged unless marked below.
This amendment does not authorize implementation; it is a draft for approval
like v1 was.
...
| **(v2)** Owner with an existing app registration | Use an already-consented enterprise app instead of relying on the shared first-party client | Can opt into app-only sign-in with a client secret and get the same analysis, without depending on the Microsoft Graph PowerShell client being present/consented in their tenant |
...
- **(v2, opt-in)** Offer a second sign-in mode — OAuth 2.0
  **client-credentials flow with a client secret** — for a user who already
  has an Entra app registration with **application** `Policy.Read.All`
  granted. The user chooses this mode explicitly and supplies tenant ID,
  client ID, and client secret via the UI; nothing about the default path
  changes.
...
- **(v2 revision)** App registration / non-device-code authentication is **no
  longer excluded**; it is now an opt-in second mode (see Goals), **limited to
  client-secret authentication in this amendment**. What remains excluded from
  this amendment:
  - **Certificate-based client assertions (RS256 JWT signing).** Not a
    permanent exclusion — recorded as a **deferred future enhancement** — but
    out of scope for this amendment because it requires RSA signing that
    Python's standard library does not provide. Adding it later would need
    its own explicit decision to accept a third-party dependency (e.g. the
    `cryptography` package), since that breaks the stdlib-only constraint
    below; it is not being made here.
...
- **(v2)** The owner has chosen client-secret authentication for that app for
  this amendment, deferring certificate-based auth to a documented future
  enhancement; and has chosen UI form-field entry for tenant ID, client ID,
  and secret over a server-side environment variable, accepting that the
  secret transits the browser and the loopback HTTP POST body (owner
...
- **A6 (v2) — App-only token is a drop-in bearer credential.** We assume
  ...
  the auth layer, widening this from the currently estimated issue count.
...
- Technical: Python 3.10+ standard library only; no third-party packages, no
  Node.js, no build step. **(v2)** This constraint is why certificate-based
  auth is deferred rather than included now.
