...
**Current status:** `DRAFT` (roadmap **v6**, pending Codex plan review and human approval — see below). **v5 remains separately approved** (`DECISION-029`) and continues to govern the completed M1/M2/M3 content and the completed project-level review below; nothing in v6 changes any of it.
**Roadmap version:** `6`
...
simplest/lowest-risk first so an easy win banks the pattern before the two
rules needing new `normalize_policy` fields and, for `ISSUE-0018`, a
