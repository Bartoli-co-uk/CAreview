...
| Field | Current value |
|---|---|
| Stage | `ISSUE_REVIEW`, active issue `ISSUE-0017`. Roadmap v6 is `APPROVED` (`DECISION-034`), adding `M4`: four new analyzer rules (`ISSUE-0015`–`ISSUE-0018`), researched against Microsoft's 2026 Conditional Access baseline and the CIS Microsoft 365 Foundations Benchmark, each using fields Graph already returns on the existing `identity/conditionalAccess/policies` call — three of the four (`ISSUE-0016`–`ISSUE-0018`) need `normalize_policy` extended to stop dropping fields it currently discards; `ISSUE-0015` needed no `normalize_policy` change. Both permitted Codex plan-review rounds ran (`ROADMAP-bb4952d6e6cc-*`, `ROADMAP-68655cc7b1e0-*`), all 8 findings fixed, and `RISK-012` (a documented false-negative limitation in `ISSUE-0018`'s rule) is accepted as residual in the same decision. M1/M2/M3 and the project-level review (`DECISION-033`) remain complete and accepted, unaffected. **`ISSUE-0015` is `COMPLETE` and merged** (started `DECISION-035`, fixture-scope amended `DECISION-036`, round-1 zero-finding sandbox residual accepted and merged `DECISION-037`) — `M4`'s first delivered issue. **`ISSUE-0016` is `COMPLETE` and merged** (started `DECISION-038`, fixture-scope amended `DECISION-039`, round-2 zero-finding sandbox residual accepted and merged `DECISION-040`) — `M4`'s second delivered issue. **`ISSUE-0017` is now started** (`DECISION-041`) and implemented on `ai/ISSUE-0017-admin-signin-frequency-rule`; `normalize_policy` gains `signInFrequency`/`persistentBrowser`; the issue's own allowed paths already anticipated the `strong_tenant.json` fixture addition, so no separate scope-amendment decision was needed. Round 0's fresh Codex review returned `CHANGES_REQUIRED` (F-001: a disabled `persistentBrowser` control with a stale `mode: "never"` falsely qualified); round 1 fixed it, but its own fresh review returned `BLOCKED` (F-001 sandbox-only; F-002 real: stale issue/handoff records after the round-1 fix). Round 2 fixed the records only (no further source change) — this was the second and final permitted repair round. Round 2's own fresh review also returned `BLOCKED` (F-001 sandbox-only; F-002 real but unresolved: `ROADMAP.md` never updated past round 0; F-003 low/advisory pre-existing README staleness), no product-code defect at any round. Repair budget now exhausted; presented to the human, `project/issues/ISSUE-0017.md`'s "Human decision required" section. `ISSUE-0018` remains `PLANNED`, not authorized to start; `M4` itself has not opened a milestone gate |
...
| Roadmap | `ROADMAP.md` is now at **v6, `APPROVED`** (`DECISION-034`, binds `68655cc7b1e0`), adding `M4`. **v5 remains separately approved** (`DECISION-029`, binds `8ea41ee`) and still governs the completed M3 content, which v6 does not change. v4 (`DECISION-015`, binds `9e5ba6d`) still governs the completed M1/M2 content. v3 (`DECISION-003`, `125d74f`) governed the completed M1 |
| Roadmap approval | `project/decisions/DECISION-003-roadmap-approval.md` (v3); `project/decisions/DECISION-015-roadmap-v4-approval.md` (v4); `project/decisions/DECISION-029-roadmap-v5-approval.md` (v5, approved directly from the round-2 review record, two round-2 findings accepted as residuals); `project/decisions/DECISION-034-roadmap-v6-approval-and-risk-012-acceptance.md` (v6, approved directly from the round-2 plan-review record, `RISK-012` accepted as residual in the same decision) |
| Active milestone | `M1` — `COMPLETE`, accepted (`DECISION-012`). `M2` — `COMPLETE`, accepted (`DECISION-023`). `M3` (React/TypeScript dashboard UI) — `COMPLETE`, accepted (`DECISION-032`). Project-level definition-of-done review — `COMPLETE`, accepted (`DECISION-033`). **`M4` (analyzer rule-set expansion) — `PLANNED`, roadmap v6 `APPROVED` (`DECISION-034`); `ISSUE-0015` `COMPLETE` and merged (`DECISION-037`), `ISSUE-0016` `COMPLETE` and merged (`DECISION-040`), `ISSUE-0017` started and in review (`DECISION-041`).** No milestone is actively in progress; `M4`'s own gate has not opened |
| Active issue | `ISSUE-0017` (analyzer rule `admin-signin-frequency`) — `REVIEWING`, branch `ai/ISSUE-0017-admin-signin-frequency-rule`, started under `DECISION-041`. Repair budget exhausted (rounds 0-2 all reviewed); round 2's fresh review returned `BLOCKED` with no product-code defect, presented to the human for the advance/merge decision. `ISSUE-0016` (analyzer rule `terms-of-use-required`) is `COMPLETE` (merged, accepted sandbox execution-evidence residual, `DECISION-040`), branch `ai/ISSUE-0016-terms-of-use-rule` merged to `main`. `ISSUE-0015` (analyzer rule `location-restriction-present`) is `COMPLETE` (merged, accepted sandbox execution-evidence residual, `DECISION-037`), branch `ai/ISSUE-0015-location-restriction-rule` merged to `main`. `ISSUE-0014` (wire the frontend build/tests into CI) is `COMPLETE` (merged, accepted sandbox execution-evidence residual, `DECISION-031`), branch `ai/ISSUE-0014-frontend-ci` merged to `main`. `ISSUE-0013`/`ISSUE-0012` are also `COMPLETE` (merged) |
| Issue repair round | `ISSUE-0017` round 2 is final — both of its two permitted repair rounds are used (round 1 fixed F-001, a real medium-severity defect where a disabled `persistentBrowser` control with a stale `mode: "never"` falsely qualified; round 2 fixed F-002, stale issue/handoff records left after the round-1 source fix) and round 2's own fresh review returned `BLOCKED` with a real-but-unresolved `ROADMAP.md`-staleness finding (F-002) plus a pre-existing README staleness advisory (F-003) — neither a product-code defect, both awaiting the human's decision (`project/issues/ISSUE-0017.md`, "Human decision required"). `ISSUE-0016`'s round-2 residual (zero content findings; blocked only by sandbox execution-evidence limits) is accepted per `DECISION-040` — both of its two permitted repair rounds were used. `ISSUE-0015`'s round-1 residual (zero content findings; blocked only by sandbox execution-evidence limits) is accepted per `DECISION-037` — only one of its two permitted repair rounds was needed. `ISSUE-0014`'s round-2 residual (zero content findings; blocked only by sandbox execution-evidence limits) is accepted per `DECISION-031`. `ISSUE-0013`'s round-2 residual (F-001: cleanup fails open after ~16-minute retry exhaustion) is accepted per `DECISION-027`, not eliminated. `ISSUE-0012`'s round-2 residual is accepted per `DECISION-025` |
...
| Latest implementation handoff | `project/handoffs/ISSUE-0017-handoff.md` (rounds 0-2, repair budget exhausted, round-2 review `BLOCKED` with no product-code defect, real check output 206 tests/compile/governance at rounds 1-2, presented to the human). `project/handoffs/ISSUE-0016-handoff.md` (rounds 0-2, complete, merged with accepted sandbox residual, real check output 195 tests/compile/governance at every round). `project/handoffs/ISSUE-0015-handoff.md` (rounds 0-1, complete, merged with accepted sandbox residual, real check output 190 tests/compile/governance). `project/handoffs/ISSUE-0013-handoff.md` (rounds 0-2, real check output for 188 Python + 91 Vitest tests at round 2). `project/handoffs/ISSUE-0012-handoff.md` (rounds 1-2, complete, merged with tracked residual). `project/handoffs/ISSUE-0011-handoff.md` (M2, rounds 0-1, complete) |
...
| Latest Codex issue review | `ISSUE-0017` round 2 (final, repair budget exhausted, presented to the human): `project/reviews/issues/ISSUE-0017-2a55a56ce255-codex.json` — `BLOCKED`, F-001 (medium, sandbox-only, same accepted class), F-002 (medium, real, **unresolved**: `ROADMAP.md`'s `M4` status prose never updated past round 0), F-003 (low, advisory, pre-existing: `README.md` stale test count) — no product-code defect at any round. Round 1: `project/reviews/issues/ISSUE-0017-72910d8b22dd-codex.json` — `BLOCKED`, F-001 (sandbox-only), F-002 (medium, real: issue/handoff records left internally contradictory after the round-1 source fix — fixed in round 2). Round 0: `project/reviews/issues/ISSUE-0017-079f5c72cb27-codex.json` — `CHANGES_REQUIRED`, F-001 (medium, real defect: `persistentBrowser.enabled` was not checked alongside `mode == "never"`, letting a disabled control with a stale `mode: "never"` falsely qualify) — fixed in round 1. `ISSUE-0016` round 2 (final, repair budget exhausted, accepted `DECISION-040`): `project/reviews/issues/ISSUE-0016-5a63f84c7a58-codex.json` — `BLOCKED`, **zero content findings**, sole blocker the review sandbox's execution-evidence limitations — same failure class as `ISSUE-0014`/`ISSUE-0015`'s accepted residuals. Real checks independently run in this task's environment at every round: compile exit 0, 195 tests passed, governance validation passed. Round 1: `project/reviews/issues/ISSUE-0016-3b2020bc30d0-codex.json` — `BLOCKED`, F-001 (sandbox-only), F-002 (medium: stale `CURRENT.md`/`ISSUE-0016.md` governance records, fixed in round 2). Round 0: `project/reviews/issues/ISSUE-0016-bfa12f76053b-codex.json` — `BLOCKED`, F-001 (sandbox-only), F-002 (low, advisory: stale README test count, fixed in round 1). `ISSUE-0015` round 1 (final, accepted `DECISION-037`): `project/reviews/issues/ISSUE-0015-31c94a824cc7-codex.json` — `BLOCKED`, **zero content findings**, sole blocker the review sandbox's execution-evidence limitations (denied `__pycache__`/socket/tempdir access) — same failure class as `ISSUE-0014`'s accepted round-2 residual. All three required checks were independently run in this task's own environment with real passing results (compile exit 0; 190 tests passed; governance validation passed) — see `project/handoffs/ISSUE-0015-handoff.md`. Round 0: `project/reviews/issues/ISSUE-0015-1ff0b987d2f7-codex.json` — `BLOCKED`, F-001 (medium: candidate-identity mismatch), F-002 (medium: `strong_tenant.json` fixture change outside allowed paths), F-003 (high: no durable start-authorization record) — all three repaired in round 1. `ISSUE-0014` round 2 (final, repair budget exhausted, accepted `DECISION-031`): `project/reviews/issues/ISSUE-0014-f63a0dadae91-codex.json` — `BLOCKED`, **zero content findings**, sole blocker the review sandbox's execution-evidence limitations. Round 1: `project/reviews/issues/ISSUE-0014-d72dbd9a5481-codex.json` — `BLOCKED`, F-001 (medium: `CURRENT.md` still round-0-shaped). Round 0: `project/reviews/issues/ISSUE-0014-c4cb4d28f9b7-codex.json` — `BLOCKED`, F-001 (medium: stale `CURRENT.md` rows). `ISSUE-0013` round 2 (final, repair budget exhausted): `project/reviews/issues/ISSUE-0013-8858858a2090-codex.json` — `BLOCKED`, F-001 (high: fails open after retry exhaustion; residual lacks human acceptance). Round 1: `project/reviews/issues/ISSUE-0013-8c273e194622-codex.json` — `BLOCKED`, F-001 (retry window too short) + F-002 (stale `CURRENT.md`). Round 0: `project/reviews/issues/ISSUE-0013-d3866851c7d6-codex.json` — `BLOCKED`, F-001 (fire-and-forget abandon). `ISSUE-0012` round 2 (final, repair budget exhausted): `project/reviews/issues/ISSUE-0012-195bd8e74688-codex.json` — `CHANGES_REQUIRED`, F-001 (high: unawaited/unscoped compensating logout). `ISSUE-0011` round 1 (final, unrelated M2 work): `project/reviews/issues/ISSUE-0011-e878cdcd979b-codex.json` — `BLOCKED`, zero findings, sole blocker the accepted sandbox execution-evidence residual |
| Completed issues | `ISSUE-0001` `23e6633`; `ISSUE-0002` `3c8fb869`; `ISSUE-0003` `065675e`; `ISSUE-0004` `9f3885b`; `ISSUE-0005` `3dc059f`; `ISSUE-0006` `d15f47c`; `ISSUE-0007` `b314d82` (merged `0c35851`, `DECISION-016`); `ISSUE-0008` `2051254` (merged `04e68ee`, `DECISION-017`); `ISSUE-0009` `7b0600f0831f68f8933b68ca0bba34f58a00b0cc` (merged `8253c1d7a754a3a967c2687c5ccc45e71794391a`, `DECISION-019`); `ISSUE-0010` `2a2d0b73e94d2635a645728e5b78f7f500c0a6b2` (merged `9d346f64422bf9bd5f89b43837a5f62f3e64d09b`, `DECISION-020`); `ISSUE-0011` `e878cdcd979b7be87ff20cc986cb16d0d457dfe0` (merged `b50cbc2fb67e8066f22ab06a03f61425dbf1a9d1`, `DECISION-022`); `ISSUE-0012` `195bd8e746884c23b4774162667ee5905f2680e1` (merged `5189959392ec2331c799199f5d70457ff361a3ba`, `DECISION-025`, out-of-band, tracked residual → `ISSUE-0013`); `ISSUE-0013` `8858858a2090aa72d8d0b14a6de64a17a447c120` (fast-forwarded to `main` at `80156d32feb6f4b85debc44897d04563bb35998a` — no merge commit; `DECISION-027`, out-of-band, accepted residual); `ISSUE-0014` `f63a0dadae917f35b328b60b1a562aa535d97d10` (merged, `DECISION-031`, accepted sandbox execution-evidence residual). All three of `ISSUE-0012`/`ISSUE-0013`/`ISSUE-0014` are now bound to `M3` under approved roadmap v5. `ISSUE-0015` `31c94a824cc7ae85e0baadc926f9cd94faaab8fc` (merged, `DECISION-037`, accepted sandbox execution-evidence residual) — `M4`'s first delivered issue under approved roadmap v6. `ISSUE-0016` `5a63f84c7a58611b1e7185f202cf131d7def2de3` (merged, `DECISION-040`, accepted sandbox execution-evidence residual) — `M4`'s second delivered issue |
| Last human decision | `DECISION-041` (`ISSUE-0017` start authorization); `DECISION-040` (`ISSUE-0016` advance and merge, accepted sandbox residual); `DECISION-039` (`ISSUE-0016` fixture-path scope amendment); `DECISION-038` (`ISSUE-0016` start authorization); `DECISION-037` (`ISSUE-0015` advance and merge, accepted sandbox residual); `DECISION-036` (`ISSUE-0015` fixture-path scope amendment, round-0 F-002); `DECISION-035` (`ISSUE-0015` start authorization, retroactive, round-0 F-003); `DECISION-034` (roadmap v6 approval and `RISK-012` acceptance, from the round-2 plan-review record); `DECISION-033` (project-level final approval, accepted from the round-2 record); `DECISION-032` (M3 milestone acceptance); `DECISION-031` (ISSUE-0014 advance and merge, accepted sandbox residual); `DECISION-030` (ISSUE-0014 start authorization); `DECISION-029` (roadmap v5 approval, approved from round-2 record); `DECISION-028` (RISK-009 acceptance, roadmap v5); `DECISION-027` (ISSUE-0013 risk acceptance and merge); `DECISION-026` (ISSUE-0013 start authorization); `DECISION-025` (ISSUE-0012 advance and merge, tracked residual); `DECISION-024` (React/Vite frontend, direct-override, out-of-band); `DECISION-023` (M2 milestone acceptance); `DECISION-022` (ISSUE-0011 advance and merge); `DECISION-021` (ISSUE-0011 start authorization); `DECISION-020` (ISSUE-0010 advance and merge); `DECISION-019` (ISSUE-0009 advance and merge); `DECISION-018` (ISSUE-0009 start authorization); `DECISION-017` (ISSUE-0008 advance and merge); `DECISION-016` (ISSUE-0007 advance and merge); `DECISION-015` (roadmap v4 approval); `DECISION-014` (secret retention, RISK-002-as-widened, tenant validation); `DECISION-013` (brief v2 approval); also `DECISION-012`..`001` |
| Open blockers | None for `M1`–`M3` (all `COMPLETE` and accepted: `DECISION-012`, `DECISION-023`, `DECISION-032`; project-level review `COMPLETE` and accepted, `DECISION-033`); `ISSUE-0015`/`ISSUE-0016` are both `COMPLETE` and merged with their sandbox-only residuals accepted (`DECISION-037`, `DECISION-040`). `ISSUE-0017`'s round-2 candidate is implemented and locally verified (206 tests, compile, governance all passing); its round-2 fresh Codex review returned `BLOCKED` with no product-code defect (real-but-unresolved `ROADMAP.md` staleness, F-002; pre-existing README staleness advisory, F-003) and the repair budget is now exhausted, so this is presented to the human rather than repaired further. `ISSUE-0018` remains unauthorized to start |
| Tracked follow-up | Live-tenant sign-in verification (M1, M2, and now M3, all auth modes) — deferred pending the human's access restrictions (`DECISION-012`), remains a protected action, not a completion gate for any milestone. `DECISION-023`'s SEC-001 (silent-renewal-after-revocation replay) and SEC-003 (unauthenticated credential-validation oracle, undocumented in `RISK-002`) are tracked, non-blocking follow-ups — see `project/milestones/M2.md` and `DECISION-023`. **CI runs the frontend build and its 91 Vitest tests on every push/PR (`ISSUE-0014`, merged `DECISION-031`).** CI had actually been red since `ISSUE-0012` merged (four backend tests checking the served `web/index.html` fail on a fresh checkout that never builds the frontend) — `ISSUE-0014`'s initial step order didn't fix this; fixed at `861f401` by building the frontend before the Python test step runs. `main`'s CI is green (confirmed via `gh run watch`). **`RISK-009` (npm build-time supply chain), `RISK-010` (onboarding regression), and `RISK-011` (abandonment fails open) are all accepted as residual** (`DECISION-028`, `DECISION-029`, `DECISION-027`) |
| Next required actor | The human, to decide `ISSUE-0017`'s disposition per `project/issues/ISSUE-0017.md`'s "Human decision required" section (accept the sandbox-residual/`ROADMAP.md`-staleness findings and authorize a metadata-only follow-up plus merge, or a different disposition) |
| Next permitted action | **Wait for the human's `ISSUE-0017` decision.** No further repair round may be attempted regardless of finding severity (`AGENTS.md`'s two-round cap is exhausted); a decision to fix `ROADMAP.md`/`README.md` afterward is a metadata-only commit, not a new implementation round, and needs no fresh Codex review. `ISSUE-0018` remains unauthorized to start |
| Actions not yet permitted | Starting `ISSUE-0018` without its own separate human start authorization; starting any other new issue/milestone/roadmap work without an explicit human decision; reopening `ISSUE-0013`'s accepted residual, `ISSUE-0014`'s accepted sandbox residual, `ISSUE-0015`'s accepted sandbox residual (`DECISION-037`), `ISSUE-0016`'s accepted sandbox residual (`DECISION-040`), `M3`'s accepted governance-record staleness (`DECISION-032`), the project-level review's accepted residual (`DECISION-033`), or `DECISION-028`'s `RISK-009`/`DECISION-029`'s `RISK-010`/`DECISION-034`'s `RISK-012` acceptances, without a fresh authorization; publication, deployment, live tenant auth/fetch (either mode), or any other protected action |
