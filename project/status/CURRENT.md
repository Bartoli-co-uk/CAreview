...
| Field | Current value |
|---|---|
| Stage | `ROADMAP_REVIEW` — roadmap v6 (DRAFT, this commit) proposes `M4`: four new analyzer rules (`ISSUE-0015`–`ISSUE-0018`), researched against Microsoft's 2026 Conditional Access baseline and the CIS Microsoft 365 Foundations Benchmark, each using fields Graph already returns on the existing `identity/conditionalAccess/policies` call but currently dropped by `normalize_policy` — no new Graph permission or endpoint. Needs no brief amendment (squarely inside brief v2's existing "data-driven rule set" framing). v6's mandatory fresh Codex plan review has not yet run — that is the immediate next step, per `./scripts/run-codex-review.sh plan <head-sha>` against this exact commit. M1/M2/M3 and the project-level review (`DECISION-033`) remain complete and accepted, unaffected by this draft; no active issue or milestone |
...
| Next required actor | The roadmap-review task, to run the fresh Codex plan review against this commit, respond to any findings (bounded per `AGENTS.md`'s planning rules — at most two revision rounds), and then present the human with the roadmap v6 approval decision. The human's own decision (approve v6, direct a fix, or reject/hold) cannot be made by an agent |
| Next permitted action | Run `./scripts/run-codex-review.sh plan <head-sha>` against this exact commit; respond to findings within the permitted revision-round budget; then present the human with the v6 approval decision. No `ISSUE-0015`–`ISSUE-0018` work may start before that approval, and no issue may start without its own separate human authorization even after v6 is approved |
| Actions not yet permitted | Starting any new issue/milestone/roadmap work without an explicit human decision; reopening `ISSUE-0013`'s accepted residual, `ISSUE-0014`'s accepted sandbox residual, `M3`'s accepted governance-record staleness (`DECISION-032`), the project-level review's accepted residual (`DECISION-033`), or `DECISION-028`'s `RISK-009`/`DECISION-029`'s `RISK-010` acceptances, without a fresh authorization; publication, deployment, live tenant auth/fetch (either mode), or any other protected action |
