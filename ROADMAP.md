...
**Current status:** `APPROVED` (roadmap **v6**, approved `DECISION-034` directly from the round-2 plan-review record — see below). **v5 remains separately approved** (`DECISION-029`) and continues to govern the completed M1/M2/M3 content and the completed project-level review below; nothing in v6 changes any of it.
**Roadmap version:** `6`
...
**Governing brief for M4:** none new — **(v6)** M4 needs no brief amendment. It is a narrower increment squarely inside brief v2's existing framing of "a transparent, data-driven rule set" (`PROJECT_BRIEF.md:68`) and does not touch any confirmed fact, assumption, or open question that brief governs. Only this roadmap version is new.
**Codex plan review:** v3 rounds recorded in Planning reconciliation below. v4: four rounds recorded below (`ROADMAP-71f7ba60b045-*`, `ROADMAP-605c282c5c81-*`, `ROADMAP-76a09c46a57d-*`, `ROADMAP-faf5ec70bf00-*`); the review/repair loop's absolute five-iteration cap (`AGENTS.md`) was reached, and the human approved directly from that record. v5: two rounds recorded below (`ROADMAP-441b4da0d3ba-*`, `ROADMAP-9bd2c0f8f6fb-*`); the human approved directly from the round-2 record (`DECISION-029`). **(v6) Round 1: `CHANGES_REQUIRED`** — `project/reviews/plans/ROADMAP-bb4952d6e6cc-codex.json`, reviewed candidate `bb4952d6e6cc7a14eda10a30c1dbcb74f09bf8cd`, 5 findings, all fixed. **Round 2: `CHANGES_REQUIRED`** — `project/reviews/plans/ROADMAP-68655cc7b1e0-codex.json`, reviewed candidate `68655cc7b1e0a63db3d6b37debf834c126bb60e0`, 3 required + 1 advisory finding, all fixed. This was the second and final permitted planning-repair round (`AGENTS.md`); per `DECISION-034`, the human approved v6 directly from this record rather than authorizing a third round — the same disposition `DECISION-015` (v4) and `DECISION-029` (v5) each used at their own revision-round caps.
**Human approval record:** `project/decisions/DECISION-003-roadmap-approval.md` (APPROVE, binds v3 at `125d74f6d4bfe85f1a727293064d0887f2d121c7`); `project/decisions/DECISION-015-roadmap-v4-approval.md` (APPROVE, binds **v4** at `9e5ba6d`); `project/decisions/DECISION-029-roadmap-v5-approval.md` (APPROVE, binds **v5** at `8ea41ee`). **(v6)** `project/decisions/DECISION-034-roadmap-v6-approval-and-risk-012-acceptance.md` (APPROVE, binds **v6** at `68655cc7b1e0a63db3d6b37debf834c126bb60e0`; also accepts `RISK-012`).
**Delivery status:** `M1` COMPLETE and accepted (`DECISION-012`); all six issues merged. `M2` is `COMPLETE` and accepted (`DECISION-023`); `ISSUE-0007` COMPLETE and merged (`DECISION-016`); `ISSUE-0008` COMPLETE and merged (`DECISION-017`); `ISSUE-0009` COMPLETE and merged (`DECISION-019`); `ISSUE-0010` COMPLETE and merged (`DECISION-020`); `ISSUE-0011` COMPLETE and merged (`DECISION-022`) — all five planned M2 issues are merged and the M2 milestone acceptance gate has passed. `M3` (React/TypeScript dashboard UI) is `COMPLETE` and accepted (`DECISION-032`) — all three of its planned issues merged (`ISSUE-0012` `DECISION-025`, `ISSUE-0013` `DECISION-027`, `ISSUE-0014` `DECISION-031`), and both rounds of the four-review milestone gate passed with zero product defects (residual governance-record staleness accepted per `DECISION-032`). The project-level definition-of-done review (`project/milestones/PROJECT.md`) is `COMPLETE` and accepted (`DECISION-033`). **(v6)** `M4` (analyzer rule-set expansion: four new CA-policy comparison rules) is **`PLANNED` and roadmap-approved** (`DECISION-034`); `M4`'s milestone gate itself has not opened. `ISSUE-0015` (`location-restriction-present`) is `COMPLETE` and merged (started `DECISION-035`, fixture-scope amended `DECISION-036`, merged `DECISION-037`). `ISSUE-0016` (`terms-of-use-required`) is `COMPLETE` and merged (started `DECISION-038`, fixture-scope amended `DECISION-039`, merged `DECISION-040`). `ISSUE-0017` (`admin-signin-frequency`) is `COMPLETE` and merged (started `DECISION-041`, both permitted repair rounds used — round 1 fixed a real defect, F-001; round 2 fixed stale governance records, F-002 — round-2 zero-product-finding sandbox residual accepted and merged `DECISION-042`). `ISSUE-0018` remains `PLANNED`, roadmap `APPROVED`, still needing its own separate human start authorization. Current state: `project/status/CURRENT.md`.

No implementation may begin until a human records approval of the exact roadmap
version and commit. Roadmap v5 remains a separately approved artifact governing
M1, M2, M3, and the completed project-level review, all of which are delivered.
**(v6)** This version is now approved (`DECISION-034`, binding
`68655cc7b1e0a63db3d6b37debf834c126bb60e0`): it adds `M4`, four issues, and
`RISK-012` (accepted as residual in the same decision). Approval of the
roadmap does not by itself start an issue — that still needs its own
separate human decision, exactly as every prior issue's start has (e.g.
`ISSUE-0014`'s own start, `DECISION-030`). `ISSUE-0015` is `COMPLETE`
(merged, accepted sandbox residual, `DECISION-037`). `ISSUE-0016` is
`COMPLETE` (merged, accepted sandbox residual, `DECISION-040`).
`ISSUE-0017` is `COMPLETE` (merged, accepted sandbox residual,
`DECISION-042`). `ISSUE-0018` is not yet authorized to start.
...
| `M3` **(v5)** | React/TypeScript dashboard UI (`frontend/`, built with Vite) replacing the vanilla-JS `web/` UI, against the unchanged `/api/policies` + `/api/analysis` contract | `M2` COMPLETE; `DECISION-024` (build-step exception) | `ISSUE-0012`, `ISSUE-0013`, and `ISSUE-0014` all COMPLETE and merged — **all three satisfied**; `python3 -m unittest discover -s tests`, `python3 -m py_compile $(git ls-files '*.py')`, `python3 scripts/validate_repo.py`, and `cd frontend && npm test` all pass at one frozen candidate, and CI now runs the frontend build/tests too (`ISSUE-0014`); the served bundle keeps `default-src 'self'` with no external asset and no new endpoint; untrusted tenant strings still render as text (JSX escaping, asserted by both `frontend/src/test/hostileMarkup.test.tsx` and `tests/test_ui_safety.py`); four blind milestone reviews ran twice (round 1 and round 2) against two frozen candidates — **zero product defects in either round**, residual governance-record staleness accepted `DECISION-032` | `COMPLETE` (`DECISION-032`) |
| `M4` **(v6)** | Analyzer rule-set expansion: four new CA-policy comparison rules (`location-restriction-present`, `terms-of-use-required`, `admin-signin-frequency`, `phishing-resistant-mfa-admins`), all reading fields Microsoft Graph already returns on the existing `identity/conditionalAccess/policies` call — no new Graph permission or endpoint. `terms-of-use-required`, `admin-signin-frequency`, and `phishing-resistant-mfa-admins` need `normalize_policy` extended to stop dropping fields it currently discards; `location-restriction-present` needs no `normalize_policy` change at all, since `includeLocations`/`excludeLocations` are already captured | `M3` COMPLETE; roadmap v6 approved | `ISSUE-0015`–`ISSUE-0018` all COMPLETE and merged; `python3 -m unittest discover -s tests`, `python3 -m py_compile $(git ls-files '*.py')`, and `python3 scripts/validate_repo.py` pass at one frozen candidate; **`cd frontend && npm ci && npm run build && npm test` also passes at that same frozen candidate** — none of the four issues touch `frontend/**`, but the dashboard consumes `/api/analysis`'s findings list, so the frontend's existing regression suite (including its hostile-markup/dangerous-sink checks) must be re-confirmed clean against the widened rule set, the same way M3's own milestone gate required it; `README.md`'s "What it checks" table and rule-count/total-weight sentence (120 → 160) match `rules.py` exactly; no existing rule's pass/fail state regresses on the current fixtures; four blind milestone reviews pass against one frozen candidate | `PLANNED` — roadmap approved (`DECISION-034`), not started |

**(v5, historical) `M3`'s milestone gate has since run and passed.** The
paragraph that previously stood here explained why `M3` was not yet marked
`COMPLETE` (its milestone gate — four blind reviews against one frozen
candidate — had not yet run, even though both delivered issues had already
passed their per-issue Codex gates). That gate has since run twice (round 1
and round 2), found zero product defects in either round, and the human
accepted the residual governance-record staleness at `DECISION-032`. `M3` is
now `COMPLETE` (see the Milestones table above and the M3 issue sequence
below) — this paragraph is removed rather than left to contradict that.
...
### M3 issue sequence **(v5)** (delivered out-of-band; milestone `COMPLETE`, `DECISION-032`)
...
### M4 issue sequence **(v6, APPROVED — `DECISION-034`)**

Four new analyzer rules, researched against Microsoft's 2026 Conditional
Access baseline recommendations and the CIS Microsoft 365 Foundations
Benchmark. Each is independently branchable/reviewable/mergeable — sequenced
simplest/lowest-risk first so an easy win (`ISSUE-0015`, no `normalize_policy`
change at all) banks the pattern before the three rules needing new
`normalize_policy` fields (`ISSUE-0016`–`ISSUE-0018`) and, for `ISSUE-0018`, a
hardcoded external fact (Microsoft's built-in phishing-resistant
authentication-strength policy ID) that must be re-verified at
implementation time rather than trusted from the issue file alone. None of
the four touch `analyzer.py`, `server.py`, or `frontend/**` — the generic
`requires`-walking scoring machinery and the `/api/analysis` response shape
are unaffected; see each issue file's own "Verified external fact" or data-
contract section for detail beyond this summary.

| Order | Issue | Objective | Depends on | Acceptance and checks | Risk | Status |
|---:|---|---|---|---|---|---|
| 15 | `ISSUE-0015` | New rule `location-restriction-present`: at least one enabled policy conditions on a named location beyond the default All/AllTrusted | Roadmap v6 approved | No `normalize_policy` change (fields already captured); `python3 -m unittest discover -s tests` passes with the new fail/pass cases; `README.md` table/weight sentence updated; no existing rule's fixture pass/fail state changes except one additive `strong_tenant.json` policy (`DECISION-036`) | Low | `COMPLETE` (merged, accepted sandbox residual, `DECISION-037`) |
| 16 | `ISSUE-0016` | New rule `terms-of-use-required`: an enabled, meaningfully-scoped policy requires Terms of Use acceptance | `ISSUE-0015` (sequencing only, not a hard dependency) | `normalize_policy` gains `grantControls.termsOfUse` (additive, safely defaulted); `tests/test_graph.py` covers the new field's defaults and round-trip; `tests/test_analyzer.py` covers fail/pass; `README.md` updated; no existing rule's fixture pass/fail state changes except one additive `strong_tenant.json` policy (`DECISION-039`) | Low | `COMPLETE` (merged, accepted sandbox residual, `DECISION-040`) |
| 17 | `ISSUE-0017` | New rule `admin-signin-frequency`: full admin-role-union coverage requiring short re-authentication (everyTime, or 1–4 hours) with no persistent browser session | `ISSUE-0016` (sequencing only) | `normalize_policy` gains `signInFrequency`/`persistentBrowser` (additive, safely defaulted); rule uses the effective-coverage algorithm (all-users-minus-`excludeRoles`, union across qualifying policies), not a verbatim copy of `mfa-admins`'s simpler pattern; nine test cases including the persistent-browser-mode-exactness nuance; one new, narrowly-scoped `strong_tenant.json` fixture entry that doesn't perturb any other rule; `README.md` updated | Medium — new fixture entry, human-confirmed threshold must not be silently changed | `COMPLETE` (started `DECISION-041`, merged `DECISION-042`) |
| 18 | `ISSUE-0018` | New rule `phishing-resistant-mfa-admins`: full admin-role-union coverage requiring Microsoft's built-in phishing-resistant authentication strength (not plain or passwordless MFA) | `ISSUE-0017` (sequencing only) | `normalize_policy` gains `grantControls.authenticationStrengthId` (additive, safely defaulted); the hardcoded strength-policy ID is re-verified against a current, cited Microsoft source at implementation time, not copied from the issue file untested; three test cases including the plain/passwordless-strength-still-fails nuance; the custom-authentication-strength limitation is documented in both `rules.py` and `README.md`; `README.md` updated | Medium — highest technical-correctness bar; documented false-negative limitation | `PLANNED` |

#### Per-issue boundaries **(v6, APPROVED)**

| Issue | Allowed paths | Explicitly out of scope | Documentation in the same change |
|---|---|---|---|
| `ISSUE-0015` | `rules.py`, `README.md`, `tests/test_analyzer.py`, `tests/fixtures/strong_tenant.json` (added by `DECISION-036`, scoped to one additive policy) | Named-location GUID/country resolution (new Graph endpoint); the other three M4 rules | README "What it checks" row |
| `ISSUE-0016` | `graph.py`, `rules.py`, `README.md`, `tests/test_graph.py`, `tests/test_analyzer.py`, `tests/fixtures/strong_tenant.json` (added by `DECISION-039`, scoped to one additive policy) | Terms of Use agreement-name resolution (new Graph endpoint); any change to existing `grantControls` keys' shape | README "What it checks" row |
| `ISSUE-0017` | `graph.py`, `rules.py`, `README.md`, `tests/test_graph.py`, `tests/test_analyzer.py`, `tests/fixtures/strong_tenant.json` | Any threshold other than the human-confirmed one; any change to the existing `sessionControls` key's shape or test contract | README "What it checks" row, stating the exact threshold |
| `ISSUE-0018` | `graph.py`, `rules.py`, `README.md`, `tests/test_graph.py`, `tests/test_analyzer.py` | Recognizing tenant-defined custom authentication strengths; any weakening of full-admin-role-union coverage | README "What it checks" row, stating the custom-strength limitation |
...
| `RISK-012` | **(v6, new; accepted `DECISION-034`)** `ISSUE-0018`'s `phishing-resistant-mfa-admins` rule only recognizes Microsoft's *built-in* phishing-resistant authentication-strength policy by a hardcoded ID. A tenant that instead defines a **custom** authentication strength restricted to the same phishing-resistant methods (FIDO2/certificate/Windows Hello) is arguably compliant but will score a FAIL on this rule — a false negative in the analyzer's output, not a gap in the tenant's actual posture | Low — analyzer-accuracy/reputational, not a security exposure in the tool itself | Jay (@Jay-cli) | **Accepted as residual per `DECISION-034`**, on the same basis already applied to `RISK-009`/`RISK-010`/`RISK-011`: document the limitation explicitly in `rules.py`'s rationale string and the `README.md` table row (required by `ISSUE-0018`'s acceptance criteria); do not attempt custom-strength recognition in this pass (would need a new Graph call to resolve the custom strength's allowed methods — its own separate design decision) | At `ISSUE-0018`; any M4 milestone security review |
...
| 7 (v6, round 1) | `ROADMAP-bb4952d6e6cc-codex.json` (`CHANGES_REQUIRED`, 5 findings, all REQUIRED/blocking) | `ROADMAP-bb4952d6e6cc-claude-response.md` — all five findings accepted and fixed: F-001 (stale pre-`DECISION-032` M3 paragraph removed), F-002 (M4 issue files now name roadmap v6/`M4` while keeping both human gates), F-003 (Terms-of-Use rule now requires `operator: "AND"` and exclusion-aware scoping), F-004 (admin-coverage rules now specify an explicit effective-coverage algorithm instead of reusing `mfa-admins`'s simpler pattern verbatim), F-005 (M4 milestone exit criteria now require the frontend build/test suite too) | First of two permitted planning-repair rounds |
| 8 (v6, round 2) | `ROADMAP-68655cc7b1e0-codex.json` (`CHANGES_REQUIRED`, 3 REQUIRED + 1 advisory) | `ROADMAP-68655cc7b1e0-claude-response.md` — all four findings accepted and fixed: F-001 (`admin-signin-frequency` now requires `persistentBrowser.mode == "never"` exactly, not merely "not always"), F-002 (`ISSUE-0018`'s custom-strength limitation reworded from "accepted" to "proposed, pending `RISK-012` acceptance"), F-003 (impossible fixed "14 other rules" counts replaced with accurate, sequencing-relative language), F-004 (M4 summary corrected — only `ISSUE-0016`–`ISSUE-0018` need new `normalize_policy` fields, not `ISSUE-0015`) | Second and final permitted planning-repair round used; presented to the human for the v6 approval decision |
