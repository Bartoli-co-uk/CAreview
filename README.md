...
The rule set lives in [`rules.py`](rules.py) — thirteen data-driven rules, each with a
severity, a weight, a rationale and a remediation string. Total weight is 145.
...
| `admin-signin-frequency` | Administrators must re-authenticate frequently (everyTime, or 1-4 hours) with no persistent browser session | medium | 10 |
...
| [`rules.py`](rules.py) | The declarative rule set: thirteen rules with severity, weight, rationale, remediation and required fields, plus the evaluability model. |
...
| [`tests/`](tests/) | 206 unit tests plus sanitized fixtures (`strong`, `weak`, `incomplete` tenants), all offline — no sign-in, no network. The frontend has its own parallel test suite (`frontend/src/test/`, run via `npm test`). |
