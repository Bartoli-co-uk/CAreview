...
`ISSUE-0014`'s own start, `DECISION-030`). `ISSUE-0015` is now started
(`DECISION-035`) and `COMPLETE` (merged, accepted sandbox residual,
`DECISION-037`). `ISSUE-0016`–`ISSUE-0018` are not yet authorized to start.
...
| 16 | `ISSUE-0016` | New rule `terms-of-use-required`: an enabled, meaningfully-scoped policy requires Terms of Use acceptance | `ISSUE-0015` (sequencing only, not a hard dependency) | `normalize_policy` gains `grantControls.termsOfUse` (additive, safely defaulted); `tests/test_graph.py` covers the new field's defaults and round-trip; `tests/test_analyzer.py` covers fail/pass; `README.md` updated | Low | `PLANNED` |
...
| `ISSUE-0016` | `graph.py`, `rules.py`, `README.md`, `tests/test_graph.py`, `tests/test_analyzer.py` | Terms of Use agreement-name resolution (new Graph endpoint); any change to existing `grantControls` keys' shape | README "What it checks" row |
