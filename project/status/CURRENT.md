...
| Open blockers | None for `M1`–`M3` (all `COMPLETE` and accepted: `DECISION-012`, `DECISION-023`, `DECISION-032`; project-level review `COMPLETE` and accepted, `DECISION-033`). `ISSUE-0016`'s round-0 candidate is implemented and locally verified (195 tests, compile, governance all passing) but has no Codex review yet. `ISSUE-0017`/`ISSUE-0018` remain unauthorized to start |
