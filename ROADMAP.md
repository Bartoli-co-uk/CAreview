...
`ISSUE-0014`'s own start, `DECISION-030`). None of `ISSUE-0015`–`ISSUE-0018`
is authorized to start yet.
...
| Order | Issue | Objective | Depends on | Acceptance and checks | Risk | Status |
|---:|---|---|---|---|---|---|
| 15 | `ISSUE-0015` | New rule `location-restriction-present`: at least one enabled policy conditions on a named location beyond the default All/AllTrusted | Roadmap v6 approved | No `normalize_policy` change (fields already captured); `python3 -m unittest discover -s tests` passes with the new fail/pass cases; `README.md` table/weight sentence updated; no existing rule's fixture pass/fail state changes | Low | `PLANNED` |
...
| Issue | Allowed paths | Explicitly out of scope | Documentation in the same change |
|---|---|---|---|
| `ISSUE-0015` | `rules.py`, `README.md`, `tests/test_analyzer.py` | Named-location GUID/country resolution (new Graph endpoint); the other three M4 rules | README "What it checks" row |
