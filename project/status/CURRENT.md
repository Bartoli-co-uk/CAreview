...
| Field | Current value |
|---|---|
| Stage | `ISSUE_REPAIR` — roadmap v5 is `APPROVED` (`DECISION-029`, binding `8ea41ee`). `ISSUE-0014` (wire the frontend build/tests into CI) was start-authorized (`DECISION-030`); round 0 (`c4cb4d2`, `project/reviews/issues/ISSUE-0014-c4cb4d28f9b7-codex.json`) returned `BLOCKED` — F-001 (medium): this file contained stale rows (Active milestone/Open blockers/Tracked follow-up) contradicting the round-0 candidate, now fixed at this commit. Round 1 repair in progress on `ai/ISSUE-0014-frontend-ci` |
