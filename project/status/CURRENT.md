...
**Resume point:** `ISSUE-0009` (`POST /api/auth/app` endpoint wiring
app-only mode to the server) is **COMPLETE**. Its round-1 candidate
`7b0600f0831f68f8933b68ca0bba34f58a00b0cc` was reviewed clean of every
...
rounds was needed — round 0 found 3 issues, since fixed: a missing durable
start-authorization record plus a stale base SHA, and two test-coverage
gaps), and the human approved advancing and merging it (`DECISION-019`).
Merged into `main` at `8253c1d7a754a3a967c2687c5ccc45e71794391a`. Required
checks re-run on the merged tree: 162 tests passed, `py_compile` clean,
`validate_repo.py` clean.

`ISSUE-0009` was itself started under an explicit human authorization
(`DECISION-018`), superseding `DECISION-017`'s prior deferral. No such
deferral is currently in effect for `ISSUE-0010`.
...
and `ISSUE-0009` are all complete and merged; `ISSUE-0010..0011` have not
started.

| Field | Current value |
|---|---|
| Stage | `IMPLEMENTATION` — `ISSUE-0007`, `ISSUE-0008`, and `ISSUE-0009` complete; `ISSUE-0010` not yet started |
...
| Active milestone | `M1` — `COMPLETE`, accepted (`DECISION-012`). `M2` — `PLANNED`, approved, in progress (`ISSUE-0007`, `ISSUE-0008`, `ISSUE-0009` of 5 complete) |
| Active issue | None. `ISSUE-0010` is the next roadmap item; start it in a new top-level Claude task per `AGENTS.md` |
| Issue repair round | None open |
| Reviewed product commit | `6311a11a48a0a7e51e83a14ca4081d431cb46698` — the frozen M1 round-2 candidate. `ISSUE-0007`'s candidate `b314d82` merged at `0c35851`. `ISSUE-0008`'s candidate `2051254` merged at `04e68ee`. `ISSUE-0009`'s candidate `7b0600f0831f68f8933b68ca0bba34f58a00b0cc` merged at `8253c1d7a754a3a967c2687c5ccc45e71794391a`. No `ISSUE-0010` product code exists |
| Latest implementation handoff | `project/handoffs/ISSUE-0009-handoff.md` (M2, rounds 0-1, complete) |
...
| Open blockers | None product-related. None process-related — `ISSUE-0010` has no recorded deferral, but per `AGENTS.md` still requires a new top-level Claude task and the standing per-issue review/repair gate before it may merge |
| Tracked follow-up | Live-tenant sign-in verification (M1) — deferred pending the human's access restrictions (`DECISION-012`); a live app-only run (M2) will be a separate protected action once `ISSUE-0010` lands. Neither is a completion gate for any issue |
| Next required actor | Whoever starts the next top-level task — `ISSUE-0010` (sign-in card mode toggle and app-only form in `web/`) is the next roadmap item |
| Next permitted action | Start `ISSUE-0010` on a new branch (e.g. `ai/ISSUE-0010-app-only-ui`) in a **new top-level Claude task** per `AGENTS.md`, base = this merge commit `8253c1d7a754a3a967c2687c5ccc45e71794391a` |
| Actions not yet permitted | Any `ISSUE-0011`+ work; publication, deployment, live tenant auth/fetch (either mode), or any other protected action |

## Verification evidence at the reviewed commit

Re-runnable from a clean checkout; see `project/milestones/M1.md` for the
commit-bound record captured at the gate, and `project/handoffs/ISSUE-0009-handoff.md`
for `ISSUE-0009`'s verification evidence.
