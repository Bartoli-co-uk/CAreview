...
| Next required actor | The human, to authorize starting one of `ISSUE-0017`/`ISSUE-0018` (roadmap v6/`M4` is approved, but no issue may start without its own separate start decision) |
| Next permitted action | **Wait for an issue-start decision**, or start `ISSUE-0017` (next in the recommended M4 sequence) once the human authorizes it. A new task should read this file, `AGENTS.md`, and `ROADMAP.md` fresh before proposing any next step |
