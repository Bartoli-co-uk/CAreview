...
| Field | Current value |
|---|---|
| Stage | `ISSUE_REVIEW`, active issue `ISSUE-0016`. Roadmap v6 is `APPROVED` (`DECISION-034`), adding `M4`: four new analyzer rules (`ISSUE-0015`–`ISSUE-0018`), researched against Microsoft's 2026 Conditional Access baseline and the CIS Microsoft 365 Foundations Benchmark, each using fields Graph already returns on the existing `identity/conditionalAccess/policies` call — three of the four (`ISSUE-0016`–`ISSUE-0018`) need `normalize_policy` extended to stop dropping fields it currently discards; `ISSUE-0015` needed no `normalize_policy` change. Both permitted Codex plan-review rounds ran (`ROADMAP-bb4952d6e6cc-*`, `ROADMAP-68655cc7b1e0-*`), all 8 findings fixed, and `RISK-012` (a documented false-negative limitation in `ISSUE-0018`'s rule) is accepted as residual in the same decision. M1/M2/M3 and the project-level review (`DECISION-033`) remain complete and accepted, unaffected. **`ISSUE-0015` is `COMPLETE` and merged** (started `DECISION-035`, fixture-scope amended `DECISION-036`, round-1 zero-finding sandbox residual accepted and merged `DECISION-037`) — `M4`'s first delivered issue. **`ISSUE-0016` is now started** (`DECISION-038`) and implemented on `ai/ISSUE-0016-terms-of-use-rule`, `normalize_policy` gains `grantControls.termsOfUse`; the same fixture-regression pattern `ISSUE-0015` hit recurred and was resolved before commit via `DECISION-039`; awaiting its fresh Codex issue review. `ISSUE-0017`/`ISSUE-0018` remain `PLANNED`, not authorized to start; `M4` itself has not opened a milestone gate |
...
| Active milestone | `M1` — `COMPLETE`, accepted (`DECISION-012`). `M2` — `COMPLETE`, accepted (`DECISION-023`). `M3` (React/TypeScript dashboard UI) — `COMPLETE`, accepted (`DECISION-032`). Project-level definition-of-done review — `COMPLETE`, accepted (`DECISION-033`). **`M4` (analyzer rule-set expansion) — `PLANNED`, roadmap v6 `APPROVED` (`DECISION-034`); `ISSUE-0015` `COMPLETE` and merged (`DECISION-037`), `ISSUE-0016` now started and in review (`DECISION-038`).** No milestone is actively in progress; `M4`'s own gate has not opened |
| Active issue | `ISSUE-0016` (analyzer rule `terms-of-use-required`) — `REVIEWING`, branch `ai/ISSUE-0016-terms-of-use-rule`, started under `DECISION-038`. Round 0 `BLOCKED` (F-001 sandbox-only, F-002 stale README count); round 1 `BLOCKED` again (F-001 sandbox-only, F-002 stale governance records); round 2 (final permitted repair) `BLOCKED` with zero content findings, sole blocker the accepted sandbox class — awaiting the human's advance/merge decision. `ISSUE-0015` (analyzer rule `location-restriction-present`) is `COMPLETE` (merged, accepted sandbox execution-evidence residual, `DECISION-037`), branch `ai/ISSUE-0015-location-restriction-rule` merged to `main`. `ISSUE-0014` (wire the frontend build/tests into CI) is `COMPLETE` (merged, accepted sandbox execution-evidence residual, `DECISION-031`), branch `ai/ISSUE-0014-frontend-ci` merged to `main`. `ISSUE-0013`/`ISSUE-0012` are also `COMPLETE` (merged) |
| Issue repair round | `ISSUE-0016` round 2 (final, both permitted rounds used): round 0 `BLOCKED` (F-001 sandbox, F-002 stale README); round 1 `BLOCKED` (F-001 sandbox, F-002 stale governance records); round 2 `BLOCKED` with zero content findings, sole blocker the accepted sandbox class — awaiting the human's advance/merge decision. `ISSUE-0015`'s round-1 residual (zero content findings; blocked only by sandbox execution-evidence limits) is accepted per `DECISION-037` — only one of its two permitted repair rounds was needed. `ISSUE-0014`'s round-2 residual (zero content findings; blocked only by sandbox execution-evidence limits) is accepted per `DECISION-031`. `ISSUE-0013`'s round-2 residual (F-001: cleanup fails open after ~16-minute retry exhaustion) is accepted per `DECISION-027`, not eliminated. `ISSUE-0012`'s round-2 residual is accepted per `DECISION-025` |
...
| Next required actor | The human, for `ISSUE-0016`'s advance/merge decision on the round-2 (final) outcome (`BLOCKED`, zero content findings, sandbox-only residual) |
| Next permitted action | **Wait for the human's ISSUE-0016 advance/merge decision.** Both permitted repair rounds are used; no further repair round may start. `ISSUE-0017`/`ISSUE-0018` remain unauthorized to start |
| Actions not yet permitted | Starting `ISSUE-0017`/`ISSUE-0018` without their own separate human start authorization; starting any other new issue/milestone/roadmap work without an explicit human decision; reopening `ISSUE-0013`'s accepted residual, `ISSUE-0014`'s accepted sandbox residual, `ISSUE-0015`'s accepted sandbox residual (`DECISION-037`), `M3`'s accepted governance-record staleness (`DECISION-032`), the project-level review's accepted residual (`DECISION-033`), or `DECISION-028`'s `RISK-009`/`DECISION-029`'s `RISK-010`/`DECISION-034`'s `RISK-012` acceptances, without a fresh authorization; publication, deployment, live tenant auth/fetch (either mode), or any other protected action |
