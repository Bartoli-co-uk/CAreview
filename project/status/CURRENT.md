...
-->
<!-- ISSUE-0014's repair budget is exhausted (round 2 of 2); it is BLOCKED
pending the human decision in project/issues/ISSUE-0014.md's "Human
decision required" section, not undergoing further automated repair. -->
...
| Field | Current value |
|---|---|
| Stage | `ISSUE_REPAIR` — roadmap v5 is `APPROVED` (`DECISION-029`, binding `8ea41ee`). `ISSUE-0014` (wire the frontend build/tests into CI) was start-authorized (`DECISION-030`). Round 0 and round 1 were both `BLOCKED` on the same governance-record finding (stale `CURRENT.md` rows), fully repaired by round 2. **Round 2** (`f63a0da`, `project/reviews/issues/ISSUE-0014-f63a0dadae91-codex.json`), the last repair round `AGENTS.md` permits: **`BLOCKED` with zero content findings** — blocked solely by the review sandbox's own execution-evidence limitations (no writable temp dir, no loopback sockets, no network), the same class of blocker `ISSUE-0011` round 1 hit. Repair budget is now exhausted; `ISSUE-0014` is presented to the human for a decision (`project/issues/ISSUE-0014.md`'s "Human decision required") rather than a third repair round |
...
| Active issue | `ISSUE-0014` (wire the frontend build/tests into CI) — `BLOCKED`, repair budget exhausted at round 2, awaiting human decision, start-authorized by `DECISION-030`, branch `ai/ISSUE-0014-frontend-ci`, starting SHA `8e864a38e03d0df90c79a469c0e1fdf740da7904`. `ISSUE-0013` — scoped device-code abandonment — is `COMPLETE` (merged, accepted residual, `DECISION-027`), out-of-band per `DECISION-026`. `ISSUE-0012` is also `COMPLETE` (merged) |
| Issue repair round | `ISSUE-0014`: both permitted repair rounds used (0 and its two repairs = rounds 0, 1, 2); budget exhausted, `BLOCKED` with zero content findings at round 2, blocked only by sandbox execution-evidence limits — awaiting human decision. `ISSUE-0013`'s round-2 residual (F-001: cleanup fails open after ~16-minute retry exhaustion) is accepted per `DECISION-027`, not eliminated. `ISSUE-0012`'s round-2 residual is accepted per `DECISION-025` |
...
| Open blockers | **`ISSUE-0014` is `BLOCKED`, repair budget exhausted at round 2, awaiting a human decision.** The round-2 review (`project/reviews/issues/ISSUE-0014-f63a0dadae91-codex.json`) found **zero content defects** — the workflow/README/CONTRIBUTING changes structurally satisfy the acceptance criteria. It blocked solely because the review sandbox itself cannot execute `validate_repo.py` (no writable temp dir), the affected backend tests (no loopback sockets), or `npm ci`/build/test (no network) — the same "sandbox execution-evidence residual" pattern this project has already accepted repeatedly (`DECISION-010`, `-016`, `-017`, `-019`, `-020`, `-022`). The real checks *were* run outside the sandbox with results recorded in `project/issues/ISSUE-0014.md`. See that file's "Human decision required" section for the three options. Separately: roadmap v5 itself is fully approved (`DECISION-029`) with no open blocker of its own. M1/M2 remain accepted and are unaffected |
| Tracked follow-up | Live-tenant sign-in verification (M1 and M2, both auth modes) — deferred pending the human's access restrictions (`DECISION-012`), remains a protected action, not a completion gate for either milestone. `DECISION-023`'s SEC-001 (silent-renewal-after-revocation replay) and SEC-003 (unauthenticated credential-validation oracle, undocumented in `RISK-002`) are tracked, non-blocking follow-ups — see `project/milestones/M2.md` and `DECISION-023`. **`ISSUE-0014`'s candidate wires the frontend build and its 91 Vitest tests into CI (`.github/workflows/validate.yml`); implemented and passing all real checks on `ai/ISSUE-0014-frontend-ci`, but `BLOCKED` pending the human decision above — not yet merged, so CI on `main` does not run the frontend build until it merges.** **`RISK-009` (npm build-time supply chain) and `RISK-010` (onboarding regression) are both accepted as residual (`DECISION-028`, `DECISION-029`).** Whether to run M3's milestone gate at all is an open question for the human (`project/milestones/M3.md`) |
| Next required actor | **The human**, to decide `ISSUE-0014`'s three presented options (accept the sandbox-evidence residual and merge; hold pending review-tooling changes; or reject/hold for other reasons) — see `project/issues/ISSUE-0014.md`'s "Human decision required" |
| Next permitted action | **Wait for the human's `ISSUE-0014` decision.** `AGENTS.md`'s two-round issue-repair limit is exhausted; no further automated repair round may run without an explicit human decision to extend past it |
