...
| Field | Current value |
|---|---|
| Stage | `MILESTONE_REVIEW` — with `M1`/`M2`/`M3` all `COMPLETE`, the human directed running `ROADMAP.md`'s project-level definition-of-done review: fresh full-project Claude and Codex general and security reviews against one final commit. No dedicated launcher mode exists for this, so it's run through the `milestone-general`/`milestone-security` machinery against a synthetic milestone ID `PROJECT` (`project/milestones/PROJECT.md`), frozen at `802ea4d` (`M3`'s acceptance commit, `main`'s tip) |
...
| Open blockers | None. `M1`, `M2`, and `M3` are all `COMPLETE` and accepted (`DECISION-012`, `DECISION-023`, `DECISION-032`). The project-level review is in progress (`project/milestones/PROJECT.md`, candidate `802ea4d`); its four fresh reviews have not yet been recorded |
...
| Next required actor | The project-level review task, to complete and record all four fresh reviews (Claude general/security, Codex general/security) against candidate `802ea4d`, then present the human with the final-approval decision |
| Next permitted action | Run the four fresh project-level reviews; on `CHANGES_REQUIRED`/`BLOCKED`, bounded repair per the same limits `M1`/`M2`/`M3` used; on completion, present the human with the final-approval decision |
